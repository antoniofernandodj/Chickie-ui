      ...
      <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 max-w-sm">
        ...
        @if (loading()) {
          ...
          <div class="flex gap-3 items-end">
            <div class="flex-1">
              <label class="block text-sm font-medium text-gray-700 mb-1">Quantidade de mesas</label>
              <input
                type="number"
                [(ngModel)]="quantidadeInput"
                min="0"
                max="200"
                class="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:border-transparent transition"
      ...
      @if (mesas().length > 0) {
        <div>
          <div class="flex items-center justify-between mb-4">
            ...
            <button
              (click)="imprimirTodos()"
              class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            @for (mesa of mesas(); track mesa) {
              <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex flex-col items-center gap-3">
                ...
                <button
                  (click)="baixar(mesa)"
                  class="w-full py-1.5 rounded-xl text-xs font-medium bg-gray-50 hover:bg-gray-100 text-gray-600 transition-colors flex items-center justify-center gap-1.5">
